...
2. **Compute the slug.** Run:
   ```bash
   node .agents/skills/design/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
Run two independent assessments. Keep their notes separate until synthesis so the deterministic scan does not anchor the design review.

Use separate sub-agents only when the user explicitly asked for delegated or parallel agent work and the current harness allows it. Otherwise run the two assessments sequentially in the current agent. Do NOT output findings to the user until the combined report is ready.
...
**Browser visualization**: optional. Use it when the user asks for a browser pass, when the target is primarily a live URL, or when the code scan is inconclusive. Otherwise keep the critique lightweight with source review plus CLI detection.
...
For multi-view targets, inject on 1-3 representative pages only when the browser pass is warranted. If injection fails, continue with CLI results only.
...
- **Suggested command**: Which command could address this (from: $design audit, $design critique, $design polish, $design harden, $design layout, $design typeset, $design colorize, $design clarify, $design distill)

#### Persona Red Flags
> *Consult [personas](personas.md)*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `PRODUCT.md`, `DESIGN.md`, or `AGENTS.md` contains design context, also generate 1-2 project-specific personas from the audience and brand info.
...
Once the report above is finalized, write it to `.impeccable/critique/` so the user can refer back, and so `$design polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .agents/skills/design/scripts/critique-storage.mjs write <slug> <body-file>
   ```
   The helper prints the absolute path it wrote.

3. **Read the trend** for context:
   ```bash
   node .agents/skills/design/scripts/critique-storage.mjs trend <slug> 5
   ```
...
- Only recommend commands from: $design audit, $design critique, $design polish, $design harden, $design layout, $design typeset, $design colorize, $design clarify, $design distill
